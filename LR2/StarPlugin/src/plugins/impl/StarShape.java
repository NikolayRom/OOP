...
public class StarShape extends AbstractShape {
    ...
    public StarShape(Point p1, Point p2, DrawingParameters params) {
        super(params);
    ...
    private Path2D createStarPath() {
        Rectangle b = getBounds();
        double centerX = b.getCenterX();
        double centerY = b.getCenterY();
        double outRadius = Math.min(b.width, b.height) / 2.0;
        double inRadius = outRadius / 2.5;

        Path2D path = new Path2D.Double();
        for (int i = 0; i < 10; i++) {
            double angle = Math.toRadians(-90 + i * 36);
            double r = (i % 2 == 0) ? outRadius : inRadius;
    ...
    @Override public boolean contains(Point p) { return createStarPath().contains(p); }
    ...
    @Override public AbstractShape cloneShape() { return new StarShape(new Point(p1), new Point(p2), getParameters().clone()); }
}
